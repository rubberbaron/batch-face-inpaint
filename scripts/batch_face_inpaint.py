...
cwd = os.getcwd()
utils_dir = os.path.join(cwd, 'extensions', 'batch-face-inpaint', 'scripts')
...
def findFaces(facecfg, image, width, height, file, totalNumberOfFaces, singleMaskPerImage, countFaces, maskSize, skip):
    rejected = 0
    masks = []
    faces_info = []
    ...
    small_image_index = 0
    small_image = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
    ...
    if facecfg.faceMode != FaceMode.ORIGINAL:
        ...
        for landmark in landmarks:
            ...
            x = computeFaceInfo(landmark, small_width, small_height, small_image_index)
            faces_info.append(x)

        faceRects = getFaceRectangles(small_image, known_face_rects, facecfg)

        for rect in faceRects:
            landmarkHull, face_info = getFacialLandmarkConvexHull(image, rect, small_width, small_height, small_image_index, facecfg)
        ...
        if not countFaces:
            ...
            for landmark in landmarks:
                ...
                convexhull = cv2.convexHull(landmark)
                faces.append(convexhull)
                faces_info.append(computeFaceInfo(landmark, small_width, small_height, small_image_index))
    ...
    new_image.paste(processed_image, (0,0))
    ...
    try:
        dilated = binary_image # in case of exception
...
def generateImages(p, facecfg, path, searchSubdir, viewResults, saveMask, pathToSave, saveToOriginalFolder, onlyMask, saveNoFace, overrideDenoising, overrideMaskBlur, invertMask, singleMaskPerImage, countFaces, keepOriginalName, pathExisting, pathMasksExisting, pathToSaveExisting, selectedTab, loadGenParams, rotation_threshold):
    suffix = ''
    info = infotext(p)
    maskSize = 0
    if selectedTab == "generateMasksTab":
        ...
        if countFaces:
            ...
            for i, file in enumerate(allFiles):
                ...
                masks, totalNumberOfFaces, faces_info, skip = findFaces(facecfg, image, width, height, file, totalNumberOfFaces, singleMaskPerImage, countFaces, maskSize, skip)
...
# EXISTING MASKS
        ...
        if pathExisting != '' and pathMasksExisting != '':
            ...
            for i, file in enumerate(allImages):
                ...
                faces_info = []
                faces_info.append({ "angle": 0, "center": (0,0) })

                geninfo = ""
                finishedImages = faceSwap(p, masks, image, finishedImages, invertMask, forced_filename, pathToSaveExisting, info, selectedTab, geninfo, faces_info, rotation_threshold)
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Column(variant='panel'):
            ...
            with gr.Tab("Existing masks",) as existingMasksTab:
                with gr.Column(variant='panel'):
                    ...
                    pathExisting = gr.Textbox(label="Images directory",value="e:/ai/larp/sketch")
                    pathMasksExisting = gr.Textbox(label="Masks directory",value="e:/ai/larp/bodymask")
                    pathToSaveExisting = gr.Textbox(label="Output directory (OPTIONAL)",value="e:/ai/larp/out1")
        ...
        return [overrideDenoising, overrideMaskBlur, path, searchSubdir, saveMask, pathToSave, saveToOriginalFolder, viewResults, saveNoFace, onlyMask, invertMask, singleMaskPerImage, countFaces, keepOriginalName, pathExisting, pathMasksExisting, pathToSaveExisting, selectedTab, faceDetectMode, face_x_scale, face_y_scale, minFace, multiScale, multiScale2, multiScale3, minNeighbors, mpconfidence, mpcount, debugSave, optimizeDetect, loadGenParams, rotation_threshold]

    def run(self, p, overrideDenoising, overrideMaskBlur, path, searchSubdir, saveMask, pathToSave, saveToOriginalFolder, viewResults, saveNoFace, onlyMask, invertMask, singleMaskPerImage, countFaces, keepOriginalName, pathExisting, pathMasksExisting, pathToSaveExisting, selectedTab, faceDetectMode, face_x_scale, face_y_scale, minFace, multiScale, multiScale2, multiScale3, minNeighbors, mpconfidence, mpcount, debugSave, optimizeDetect, loadGenParams, rotation_threshold):
        ...
        finishedImages = generateImages(p, facecfg, path, searchSubdir, viewResults, saveMask, pathToSave, saveToOriginalFolder, onlyMask, saveNoFace, overrideDenoising, overrideMaskBlur, invertMask, singleMaskPerImage, countFaces, keepOriginalName, pathExisting, pathMasksExisting, pathToSaveExisting, selectedTab, loadGenParams, rotation_threshold)
